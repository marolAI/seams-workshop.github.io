...
* Which formats for which tasks?
    ...
    -Discus how text files impact the perfomance performance of your system because they have to be parsed every time. Text files also    
    ...
    -Columar formats offer advantage in terms of query speed e.g Select * from where ____ and where Column >= 10
    ...
    In database technologies such as Hadoop, SQL ecosystem we are used to be thinking about delimited files such as csv, and tsv. We can also think about json records as long as each line is its own 
   jason datum.
    ...
    Tasks involving storage and retrieval of hierachical data will have Database Technologies together with the standard query language (sql) being relevant.For data analysis tools such as 
    R and python being mostly used.
